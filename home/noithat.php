...
if(isset($_GET['idcategory'])){
    ...
    $result = $conn->query("SELECT nd.hoten,co.dia_chi,bd.mo_ta,bdp.gia,ha.anh_dai_dien,ha.anh,nd.id_nd FROM cho_o co inner join nguoi_dung nd inner join bai_dang bd inner join bai_dang_phong bdp inner join phong p INNER join hinh_anh ha ON co.id_bd = bd.id_bd and co.id_nd = nd.id_nd and nd.id_nd = bd.id_nd and bdp.id_bd = bd.id_bd and p.id_pt = ha.id_pt and p.id_cho = co.id_cho where ha.anh_dai_dien = 1 and co.id_loai = $idcategory");
    ...
    $result = $conn->query("SELECT DISTINCT tn.id_pt, nd.hoten, co.dia_chi, bd.mo_ta, bdp.gia, ha.anh_dai_dien, ha.anh FROM phong p JOIN tien_nghi tn ON tn.id_pt = p.id_pt JOIN cho_o co ON p.id_cho = co.id_cho JOIN nguoi_dung nd ON co.id_nd = nd.id_nd JOIN bai_dang bd ON nd.id_nd = bd.id_nd JOIN bai_dang_phong bdp ON bd.id_bd = bdp.id_bd JOIN hinh_anh ha ON p.id_pt = ha.id_pt WHERE ha.anh_dai_dien = 1 AND tn.ten_tiennghi LIKE '%Có nội thất%' GROUP BY p.id_pt");
} 
$category = $conn->query("SELECT * FROM loai");
$phuong = $conn->query("SELECT * FROM phuong");
$tiennghi = $conn->query("SELECT * FROM tien_nghi");
  ...
  if (isset($_REQUEST['search'])) {
    ...
    if (empty($search)) {
        ...
        // Phan dung vong lap while show du lieu
            ...
            $sql=$conn->query ("SELECT p.id_pt, nd.hoten,co.dia_chi,bd.mo_ta,bdp.gia,ha.anh_dai_dien,ha.anh,nd.id_nd FROM cho_o co inner join nguoi_dung nd inner join bai_dang bd inner join bai_dang_phong bdp inner join phong p INNER join hinh_anh ha ON co.id_bd = bd.id_bd and co.id_nd = nd.id_nd and nd.id_nd = bd.id_nd and bdp.id_bd = bd.id_bd and p.id_pt = ha.id_pt and p.id_cho = co.id_cho WHERE ha.anh_dai_dien = 1 AND co.dia_chi like  '%$search%'");
