...
$sql = "SELECT bd.id_bd,p.id_pt, nd.hoten,co.dia_chi,bd.mo_ta,bdp.gia,ha.anh_dai_dien,ha.anh,nd.id_nd,l.ten_loai,co.dien_tich FROM cho_o co inner join nguoi_dung nd inner join bai_dang bd inner join bai_dang_phong bdp inner join phong p INNER join hinh_anh ha INNER JOIN loai l
        ON co.id_loai = l.id_loai and co.id_bd = bd.id_bd and co.id_nd = nd.id_nd and nd.id_nd = bd.id_nd and bdp.id_bd = bd.id_bd and p.id_pt = ha.id_pt and p.id_cho = co.id_cho";
$result_map = $conn->query($sql);


if (isset($_REQUEST['address'])) {
    $search = addslashes($_GET['address']);
    if (empty($search)) {
        echo "<span>sáas</span>";
        ...
        // Phan dung vong lap while show du lieu
            ...
            $sql2=$conn->query ("SELECT bd.id_bd,p.id_pt, nd.hoten,co.dia_chi,bd.mo_ta,bdp.gia,ha.anh_dai_dien,ha.anh,nd.id_nd FROM cho_o co inner join nguoi_dung nd inner join bai_dang bd inner join bai_dang_phong bdp inner join phong p INNER join hinh_anh ha
        ON  co.id_bd = bd.id_bd and co.id_nd = nd.id_nd and nd.id_nd = bd.id_nd and bdp.id_bd = bd.id_bd and p.id_pt = ha.id_pt and p.id_cho = co.id_cho WHERE ha.anh_dai_dien = 1 and bd.trang_thai = 1 AND co.dia_chi like  '%$search%'");
