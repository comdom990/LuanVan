...
<!-- Thêm Leaflet Routing Machine CSS và JS -->
        ...
        <div class="search-menu">
                <div class="search-list">
                    <div class="search_filter">
                        <div class="search">
                        <form action="./search_map_2.php" method="GET" class="d-flex " role="search">
    ...
    <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        var customIcon = L.icon({
                            iconUrl: '../img/marker_user.png', // Đường dẫn đến hình ảnh marker
                            iconSize: [80, 50], // Kích thước của icon [width, height]
                            iconAnchor: [39, 72], // Điểm neo của icon
                    ...
                    if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(function(position) {
                           ...
                           // Lấy địa chỉ từ tọa độ của người dùng
                           var userLocation = L.marker([latitude, longitude], { icon: customIcon }).addTo(map)
                        ...
                        map.setView([latitude, longitude], 13);
                            ...
                            // Tính toán khoảng cách và tìm các địa điểm gần nhất
                            let closestMarker = null;
                            let closestDistance = Infinity;
                            markers.forEach(function(marker) {
                                    var distance = getDistance(latitude, longitude, marker.getLatLng().lat, marker.getLatLng().lng);
                       ...
                       // Hiển thị địa điểm gần nhất
                       if (closestMarker) {
                        getRoute(latitude, longitude, closestMarker.getLatLng().lat, closestMarker.getLatLng().lng);

                                // Chỉ đường đến địa điểm gần nhất
                            }
                        }, function() {
                    ...
                    let routeControl; // Khai báo biến routeControl ở cấp toàn cục
                    // Hàm chỉ đường từ vị trí người dùng đến địa điểm gần nhất
