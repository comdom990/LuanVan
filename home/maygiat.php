...
if(isset($_GET['idcategory'])){
    ...
    $result = $conn->query("SELECT DISTINCT tn.id_pt, nd.hoten, co.dia_chi, bd.mo_ta, bdp.gia, ha.anh_dai_dien, ha.anh FROM phong p JOIN tien_nghi tn ON tn.id_pt = p.id_pt JOIN cho_o co ON p.id_cho = co.id_cho JOIN nguoi_dung nd ON co.id_nd = nd.id_nd JOIN bai_dang bd ON nd.id_nd = bd.id_nd JOIN bai_dang_phong bdp ON bd.id_bd = bdp.id_bd JOIN hinh_anh ha ON p.id_pt = ha.id_pt WHERE ha.anh_dai_dien = 1 AND tn.ten_tiennghi LIKE '%Máy giặt%' GROUP BY p.id_pt");
} 
